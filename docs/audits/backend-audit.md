...
2. Repair recurring-occurrence state semantics in two isolated changes: preserve overrides through close/reopen, then introduce a representation that distinguishes inherited values from explicit null clears.
3. Externalize Firebase and database credentials, default optional integrations deliberately, and add a clean-image startup smoke test using PostgreSQL.
4. Add a required pull-request backend workflow that runs the unit suite, real PostgreSQL/Flyway/API contract tests, OpenSpec validation, dependency/security checks, and a container health smoke test.
...
| BA-021 | Medium | High | Performance/scalability | taska-backend/src/main/java/com/taska/domain/task/Task.java:70-78; taska-backend/src/main/java/com/taska/domain/task/TaskService.java:64-85; all collection controllers return List | Task list fallbacks are unbounded and labels are always eager. No collection endpoint offers pagination or a maximum range. | Query count, transferred rows, label joins/selects, heap use, and serialization time grow with the workspace. This is a high-confidence risk, not a measured production defect; row counts and SQL telemetry are missing. | First capture row counts/query metrics, make high-cardinality task/comment queries bounded, and fetch labels deliberately for DTO projections/pages rather than globally eager. | Large | Query-count integration test |
...
| POST /tasks/{id}/close | B/domain/task/TaskController.java:146-152 | TaskService.closeTask; all task views | TaskaApi.closeTask; views/widgets | MCP complete_task calls service directly | TaskService mutation/MCP unit tests; no REST HTTP test | Active on both clients | High | MCP sync bypass BA-012. |
...
- One confirmed active query-contract failure remains: Android date versus backend singleDate (BA-001).
- The frontend has adapted to one backend implementation detail rather than the documented name: the task query's date field is serialized as singleDate.
...
2. In a small compatibility change, expose date explicitly, accept singleDate temporarily, reject conflicting/partial ranges, and fix show_completed behavior (BA-001, BA-007). Update Angular to the canonical date name. Keep this separate from recurrence changes.
...
12. Add cross-field invariants one aggregate at a time: comments exactly-one target, project acyclicity, task project/parent coherence, then recurrence coherence (BA-015). Each migration should include preflight queries, representative upgrade fixtures, and a policy for existing invalid rows (BA-030).
...
15. Instrument database query counts/latency and row counts, then bound high-cardinality lists (BA-021). Do not combine query behavior changes with API contract fixes.
16. Batch planning-calendar rules and add the missing rule foreign-key index after measuring/explaining the migration (BA-022).
17. Reconcile the dueAt specifications and replace incompatible OpenSpec skills before the next workflow transition (BA-024, BA-025).
18. Remove raw token logging, correct executable documentation, and pin build tooling (BA-027, BA-028, BA-029).
19. Only after access-log review, deprecate any endpoint candidates and remove the confirmed internal dead methods. Deprecation, telemetry observation, and deletion should be separate releases.

Changes specifically not to combine:

- Date/query compatibility, Firebase secrets, and recurrence persistence are independent high-risk changes.
...
- Do not paginate every small reference list immediately. Projects, labels, and calendars may remain acceptably small in a mono-user workspace; first bound tasks/comments and measure BA-021.
