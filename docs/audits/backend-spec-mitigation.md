...
|-----------|------------------|----------|----------------|
| Nullabilité absente dans la spec | L'API renvoie `null` alors que le schéma n'autorise que `string`, `integer` ou `boolean`. | `ProjectDto.parentId`, `FilterDto.projectId`, `FilterDto.hasDate`, `TaskDto.*`, `TimeEntryDto.notes`, `CommentDto.taskId` | Déclarer explicitement les champs nullable (`type: [string, "null"]`, `type: [integer, "null"]`, etc.) dans les schémas OpenAPI. |
...
| Réponses 500 inattendues | L'API renvoie une erreur serveur sur des requêtes conformes à la spec. | `/labels`, `/comments`, `/filters`, `/projects`, `/sections`, `/time-entries` | Corriger le backend : une requête valide ne doit jamais provoquer un 500. |
| Gestion incorrecte des ressources inexistantes | Certaines requêtes stateful produisent un comportement inattendu. | `/tasks/None`, `/sections/None` | Retourner systématiquement un 404 ou 400 documenté, jamais un 500. Vérifier également les liens stateful générés. |
