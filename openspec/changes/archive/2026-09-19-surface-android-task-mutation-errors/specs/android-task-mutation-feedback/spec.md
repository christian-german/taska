...
The Android task-detail experience SHALL keep the loaded task visible when a mutation fails and
SHALL present the failure as transient, accessible feedback. It SHALL prefer a non-blank Problem
Details `detail` returned by the backend and SHALL use a safe fallback when no actionable detail is
available. After presentation begins, the failure SHALL be consumed so unrelated recomposition does
not display it repeatedly.
