...
The system SHALL allow a task and its applicable recurring-task occurrence representation to have no manual `priority`. A supplied manual priority SHALL continue to satisfy the existing priority-range validation. Task creation, update, persistence, and task representations SHALL preserve an absent priority as `null` rather than assigning priority `4`. A complete `TaskUpdateRequest` with `priority: null` SHALL clear a base task's manual priority; occurrence priority overrides SHALL remain readable but SHALL NOT be writable, including explicit null.
...
- **GIVEN** a non-recurring task has an assigned `scheduledAt` and an independent `dueAt`
...
- **GIVEN** a non-recurring task has an assigned `scheduledAt` and no `dueAt`
- **WHEN** the user removes the schedule from the Android, web, or Tauri task-detail interface
- **THEN** the client SHALL persist `scheduledAt` as `null`
- **AND** the task SHALL continue to have no deadline

#### Scenario: Clear only the scheduled time on web or Tauri
- **GIVEN** a non-recurring task has a scheduled date and time
...
- **GIVEN** a non-recurring task has an assigned `scheduledAt`
...
The system SHALL expose a recurring occurrence's effective dueAt only when persisted occurrence state contains a historical deadline override. Virtual occurrences and states without that override SHALL expose dueAt: null. Occurrence writes SHALL reject dueAt, including explicit null, without changing stored historical values.
...
- **THEN** its representation SHALL contain dueAt: null

#### Scenario: Reject a new occurrence deadline
- **WHEN** a client attempts a THIS_ONLY update containing dueAt
- **THEN** the update SHALL fail without changing the occurrence or series

#### Scenario: Preserve a historical deadline
- **WHEN** an occurrence with an existing dueAt override is moved, completed or reopened
- **THEN** it SHALL continue returning that historical deadline unchanged
