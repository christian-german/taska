...
class TaskDetailCompletionControlTest {
  ...
  fun activeControlIsUncheckedAccessibleAndInteractive() {
    var clicks = 0
    composeRule.setContent { CompletionControl(false, true, 1, { clicks++ }) }

    composeRule
      .onNodeWithTag("task-detail-completion")
      .assertIsOff()
      .assertIsEnabled()
  ...
  fun completedControlIsCheckedAndOffersReopenAction() {
    composeRule.setContent { CompletionControl(true, true, null, {}) }
  ...
  fun pendingControlIsDisabled() {
    composeRule.setContent { CompletionControl(false, false, null, {}) }
