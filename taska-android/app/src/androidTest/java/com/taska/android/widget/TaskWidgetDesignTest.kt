...
class TaskWidgetDesignTest {
  ...
  @Test
  fun `widget layout uses a rounded Taska card and circular completion controls`() {
  ...
  @Test
  fun `widget supports long task titles with bounded rows`() {
    val root = LayoutInflater.from(context).inflate(R.layout.task_widget, null)
    val title = root.findViewById<TextView>(R.id.widget_task_0)

    title.text =
      "A deliberately long Taska task title that must remain readable without escaping the rounded widget card"
  ...
  @Test
  fun `today widget reuses the widget card and completed control assets`() {
    val root = LayoutInflater.from(context).inflate(R.layout.task_widget, null)
  ...
  @Test
  fun `week widget has a scrollable list and centered date headers`() {
    val root = LayoutInflater.from(context).inflate(R.layout.week_task_widget, null)
    val header =
      LayoutInflater.from(context).inflate(R.layout.week_widget_date_header, null) as TextView
  ...
  @Test
  fun `today widget hides count status and task separators by default`() {
    val root = LayoutInflater.from(context).inflate(R.layout.task_widget, null)
  ...
  @Test
  fun `today rows provide hidden outlined appointment indicators`() {
    val root = LayoutInflater.from(context).inflate(R.layout.task_widget, null)
    ...
    assertEquals(
      ...
      indicator.layoutParams.width.toFloat(),
      0.01f,
    )
  ...
  @Test
  fun `widget color resources adapt between light and dark system themes`() {
    ...
    assertTrue(
      contrast(
        light.getColor(R.color.widget_overdue_ink),
        light.getColor(R.color.widget_surface),
      ) >= 4.5
    )
    assertTrue(
      contrast(dark.getColor(R.color.widget_overdue_ink), dark.getColor(R.color.widget_surface)) >=
        4.5
    )
  }

  @Test
  fun `overdue widget text is red and bold without changing its content`() {
    val styled = overdueWidgetText(context, "08:30  Follow up") as Spanned

    assertEquals("08:30  Follow up", styled.toString())
    assertEquals(
      context.getColor(R.color.widget_overdue_ink),
      styled.getSpans(0, styled.length, ForegroundColorSpan::class.java).single().foregroundColor,
    )
    assertEquals(
      Typeface.BOLD,
      styled.getSpans(0, styled.length, StyleSpan::class.java).single().style,
    )
  }
