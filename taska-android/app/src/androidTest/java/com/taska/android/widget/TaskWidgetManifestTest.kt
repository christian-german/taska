...
class TaskWidgetManifestTest {
  @Test
  fun `widget provider is declared with its metadata`() {
  ...
  @Test
  fun `today widget provider is declared with its metadata`() {
    val context = ApplicationProvider.getApplicationContext<android.content.Context>()
    val info =
      context.packageManager.getReceiverInfo(
  ...
  @Test
  fun `week collection service is private and requires RemoteViews binding permission`() {
    val context = ApplicationProvider.getApplicationContext<android.content.Context>()
    val info =
      context.packageManager.getServiceInfo(
        ComponentName(context, WeekTaskWidgetService::class.java),
        0,
      )
