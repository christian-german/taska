    ...
    if (taskId != null) {
      val tapPendingIntent =
        TaskStackBuilder.create(this).run {
          ...
          if (projectId != null) {
            addNextIntent(
              Intent(this@TaskaFirebaseMessagingService, ProjectActivity::class.java).apply {
                putExtra("project_id", projectId)
              }
            )
          }
          addNextIntent(
            Intent(this@TaskaFirebaseMessagingService, TaskDetailActivity::class.java).apply {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
      val channel =
        NotificationChannel(
          ...
          .apply { description = "Notifications de rappel pour les tâches à venir" }
