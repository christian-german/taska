...
object RetrofitClient {
  ...
  val api: TaskaApi by lazy {
    val authInterceptor = Interceptor { chain ->
      ...
      val token =
        am.getAccountsByType(AuthConfig.ACCOUNT_TYPE).firstOrNull()?.let { account ->
          am.blockingGetAuthToken(account, AuthConfig.AUTH_TOKEN_TYPE, false)
        }
      val request =
        if (token != null) {
    ...
    val tokenAuthenticator = okhttp3.Authenticator { _, response ->
      ...
      val am = AccountManager.get(appContext)
      val account =
        am.getAccountsByType(AuthConfig.ACCOUNT_TYPE).firstOrNull() ?: return@Authenticator null
