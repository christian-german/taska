...
fun WithDrawer(
  ...
  ModalNavigationDrawer(
    ...
    Box(
      modifier =
        Modifier.fillMaxSize().pointerInput(drawerState) {
...
private fun ProjectDrawer(
  ...
  Column(
    modifier =
      Modifier.fillMaxHeight().fillMaxWidth(0.85f).frostedChrome().statusBarsPadding().pointerInput(
        Unit
      ) {
...
private fun ProjectTreeItem(
  ...
  Row(
    ...
    if (hasChildren) {
      Icon(
        imageVector =
          if (isExpanded) Icons.Outlined.KeyboardArrowDown else Icons.Outlined.ChevronRight,
        contentDescription = null,
        modifier = Modifier.size(20.dp).clickable(onClick = { onToggleExpand(project.id) }),
        tint = TextSecondary,
      )
    } else {
