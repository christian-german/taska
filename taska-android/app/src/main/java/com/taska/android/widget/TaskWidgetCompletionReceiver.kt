...
internal enum class CompletionWidget {
  WEEK,
  TODAY,
}
...
private class RemoteViewsCompletionPresentation(private val context: Context) :
      ...
      val views =
        RemoteViews(context.packageName, R.layout.task_widget).apply {
          setImageViewResource(
            target.controlId,
            if (checked) R.drawable.widget_completion_checked
...
class TaskWidgetCompletionReceiver : BroadcastReceiver() {
  override fun onReceive(context: Context, intent: Intent) {
    ...
    if (action == ACTION_OPEN) {
      ...
      context.startActivity(
        Intent(context, com.taska.android.TaskDetailActivity::class.java).apply {
    ...
    CoroutineScope(Dispatchers.IO).launch {
      try {
        ...
        // The presentation has already been restored; BroadcastReceivers cannot surface request
        // errors.
      } finally {
        pending.finish()
      }
  ...
  companion object {
    ...
    private fun completionTarget(intent: Intent, taskId: String): CompletionTarget? {
      ...
      val widget =
        when (intent.getStringExtra(EXTRA_WIDGET_TYPE)) {
      ...
      return CompletionTarget(
        ...
        controlId,
        taskId,
        intent.getStringExtra(EXTRA_OCCURRENCE),
      )
    }
