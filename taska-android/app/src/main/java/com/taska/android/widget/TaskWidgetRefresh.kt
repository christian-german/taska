  ...
  companion object {
    fun from(tasks: List<TaskDto>): TodayWidgetGroups {
      ...
      return TodayWidgetGroups(
        ...
        dividerAfterRow =
          (overdueCount - 1).takeIf { overdueCount > 0 && overdueCount < tasks.size },
      )
    }
  }
}
...
object TaskWidgetRefresh {
  ...
  private val appointmentIds =
    intArrayOf(
      R.id.widget_appointment_0,
      R.id.widget_appointment_1,
      R.id.widget_appointment_2,
      R.id.widget_appointment_3,
      R.id.widget_appointment_4,
      R.id.widget_appointment_5,
      R.id.widget_appointment_6,
      R.id.widget_appointment_7,
    )
  private val dividerIds =
    intArrayOf(
      R.id.widget_divider_0,
      R.id.widget_divider_1,
      R.id.widget_divider_2,
      R.id.widget_divider_3,
      R.id.widget_divider_4,
      R.id.widget_divider_5,
  ...
  suspend fun refresh(context: Context) {
    ...
    widgets
      ...
      .forEach { (type, widgetIds) -> refreshType(context, manager, type, widgetIds) }
  }
  ...
  private suspend fun refreshType(
    ...
    try {
      ...
      widgetIds.forEach { widgetId ->
        ...
        if (type == WidgetType.WEEK)
          manager.notifyAppWidgetViewDataChanged(widgetId, R.id.widget_week_list)
      }
    } catch (_: Exception) {
      widgetIds.forEach { widgetId ->
        manager.updateAppWidget(
          widgetId,
          render(context, type, widgetId, emptyList(), range, "Unable to refresh tasks"),
        )
        if (type == WidgetType.WEEK)
          manager.notifyAppWidgetViewDataChanged(widgetId, R.id.widget_week_list)
      }
  ...
  private fun render(
    ...
    if (type == WidgetType.WEEK) renderWeek(context, widgetId, tasks, range, error)
    else
      RemoteViews(context.packageName, R.layout.task_widget).apply {
        ...
        setViewVisibility(
          R.id.widget_overdue_header,
          if (groups.showOverdueHeader) View.VISIBLE else View.GONE,
        )
        dividerIds.forEachIndexed { index, dividerId ->
          setViewVisibility(
            dividerId,
            if (index == groups.dividerAfterRow) View.VISIBLE else View.GONE,
          )
        }
  ...
  private fun renderWeek(
    ...
    val serviceIntent =
      Intent(context, WeekTaskWidgetService::class.java).apply {
        putExtra(AppWidgetManager.EXTRA_APPWIDGET_ID, widgetId)
        data = android.net.Uri.parse("taska://widget/week/$widgetId")
      }
    val actionTemplate =
      Intent(context, TaskWidgetCompletionReceiver::class.java).apply {
  ...
  private fun titleFor(type: WidgetType, range: Pair<LocalDate, LocalDate>): String =
    when (type) {
      WidgetType.WEEK ->
        "Taska · ${range.first.format(DateTimeFormatter.ofPattern("MMM d"))}–${range.second.format(DateTimeFormatter.ofPattern("MMM d"))}"
      WidgetType.TODAY ->
        "Taska · Today · ${range.first.format(DateTimeFormatter.ofPattern("MMM d"))}"
    }
  ...
  private fun RemoteViews.bindTask(
    context: Context,
    widgetId: Int,
    index: Int,
    task: TaskDto,
    ...
    if (task.isOverdueWidgetTask()) setOverdueText(context, taskIds[index], text)
    else setTextViewText(taskIds[index], text)
    val completed = canShowCompleted && task.isCompleted == true
    setImageViewResource(
      checkIds[index],
      if (completed) R.drawable.widget_completion_checked else R.drawable.widget_completion_empty,
    )
    setInt(
      taskIds[index],
      "setPaintFlags",
      if (completed) Paint.ANTI_ALIAS_FLAG or Paint.STRIKE_THRU_TEXT_FLAG
      else Paint.ANTI_ALIAS_FLAG,
    )
    bindAppointmentIndicator(context, appointmentIds[index], task.type)

    val action =
      if (completed) TaskWidgetCompletionReceiver.ACTION_REOPEN
      else TaskWidgetCompletionReceiver.ACTION_COMPLETE
    val completion =
      Intent(context, TaskWidgetCompletionReceiver::class.java).apply {
        this.action = action
        ...
        putExtra(
          TaskWidgetCompletionReceiver.EXTRA_WIDGET_TYPE,
          TaskWidgetCompletionReceiver.WIDGET_TYPE_TODAY,
        )
        putExtra(TaskWidgetCompletionReceiver.EXTRA_CONTROL_ID, checkIds[index])
        data =
          android.net.Uri.parse(
            "taska://widget/${if (completed) "reopen" else "complete"}/${task.id}/${task.occurrenceScheduledAt ?: "single"}"
          )
      }
    ...
    val open =
      Intent(context, TaskDetailActivity::class.java).apply {
        ...
        data =
          android.net.Uri.parse(
            "taska://widget/task/${task.id}/${task.occurrenceScheduledAt ?: "single"}"
          )
      }
    val pendingOpen =
  ...
  internal fun todayTaskText(task: TaskDto): String {
    ...
    return if (task.allDay) "$day  ${task.content}"
    else "$day  ${formatTime(scheduled)} ${task.content}"
  }
  ...
  private fun RemoteViews.bindAppointmentIndicator(context: Context, viewId: Int, type: String?) {
    ...
    setContentDescription(
      viewId,
      if (visible) context.getString(R.string.widget_appointment) else null,
    )
  }

  internal fun currentDay(today: LocalDate = LocalDate.now()): Pair<LocalDate, LocalDate> =
    today to today
    ...
    val filtered =
      tasks
        ...
        .filter { task ->
          ...
          val inRange =
            date in range.first..range.second && (includeCompleted || task.isCompleted != true)
          overdue || inRange
        }
        .sortedWith(
          compareBy<TaskDto>({ scheduledDate(it.scheduledAt!!, zone) >= today }, { it.scheduledAt })
        )
  ...
  private fun scheduleNextDayRefresh(context: Context) {
    ...
    alarmManager.setAndAllowWhileIdle(
      ...
      nextLocalDayBoundary().toEpochMilli(),
      dayRefreshIntent(context),
    )
  }
  ...
  private fun dayRefreshIntent(context: Context): PendingIntent =
    PendingIntent.getBroadcast(
      context,
      DAY_REFRESH_REQUEST_CODE,
      Intent(context, WidgetDayChangeReceiver::class.java)
        .setAction(WidgetDayChangeReceiver.ACTION_REFRESH_FOR_NEW_DAY),
      PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE,
    )

  private fun scheduledDate(value: String, zone: ZoneId = ZoneId.systemDefault()): LocalDate =
