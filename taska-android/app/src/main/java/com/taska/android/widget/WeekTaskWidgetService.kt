...
internal object WeekWidgetItems {
  ...
  fun taskText(
    ...
    val time =
      Instant.parse(task.scheduledAt!!)
        .atZone(zone)
...
internal object WeekWidgetOptimisticState {
  ...
  private fun key(target: CompletionTarget) =
    "${target.widgetId}:${target.taskId}:${target.occurrenceScheduledAt.orEmpty()}"

  fun set(context: Context, target: CompletionTarget, checked: Boolean) =
    context
      .getSharedPreferences(PREFS, Context.MODE_PRIVATE)
      .edit()
      .putBoolean(key(target), checked)
      .apply()
...
class WeekTaskWidgetService : RemoteViewsService() {
  ...
  private class Factory(private val context: Context, private val widgetId: Int) :
    RemoteViewsFactory {
    ...
    override fun onDataSetChanged() {
      items = WeekWidgetItems.build(WeekWidgetDataStore.load(context, widgetId))
    }
    ...
    override fun getViewAt(position: Int): RemoteViews =
      when (val item = items[position]) {
        WeekWidgetItem.OverdueHeader ->
          RemoteViews(context.packageName, R.layout.week_widget_date_header).apply {
            setOverdueText(
              context,
        ...
        is WeekWidgetItem.DateHeader ->
          RemoteViews(context.packageName, R.layout.week_widget_date_header).apply {
            setTextViewText(R.id.widget_date_header, WeekWidgetItems.header(item.date))
          }
        is WeekWidgetItem.Task ->
          RemoteViews(context.packageName, R.layout.week_widget_task_row).apply {
            val text = WeekWidgetItems.taskText(item.task)
            if (item.task.isOverdueWidgetTask())
              setOverdueText(context, R.id.widget_task_text, text)
            ...
            setViewVisibility(
              ...
              if (appointment) View.VISIBLE else View.GONE,
            )
            ...
            setImageViewResource(
              ...
              if (WeekWidgetOptimisticState.isChecked(context, widgetId, item.task))
                R.drawable.widget_completion_checked
            ...
            val completion =
              Intent().apply {
                ...
                putExtra(
                  TaskWidgetCompletionReceiver.EXTRA_OCCURRENCE,
                  item.task.occurrenceScheduledAt,
                )
                putExtra(TaskWidgetCompletionReceiver.EXTRA_WIDGET_ID, widgetId)
                putExtra(
                  TaskWidgetCompletionReceiver.EXTRA_WIDGET_TYPE,
                  TaskWidgetCompletionReceiver.WIDGET_TYPE_WEEK,
                )
