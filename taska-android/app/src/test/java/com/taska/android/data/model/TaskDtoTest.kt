 ...
 *
 * Il n'existe pas de Mapper séparé dans ce projet : le backend retourne le DTO déjà fusionné
 * (parent + overrides de l'instance). Ces tests vérifient que la data class expose correctement les
...
class TaskDtoTest {
  ...
  fun `givenNonRecurringTaskWithNullOccurrenceScheduledAt_whenCreated_thenNoException`() {
    assertDoesNotThrow { buildTaskDto(isRecurring = false, occurrenceScheduledAt = null) }
  }
