...
class TaskWidgetCompletionReceiverTest {
  ...
  @Test
  fun `reopens a completed non-recurring task`() = runTest {
  ...
  @Test
  fun `reopens only the selected recurring occurrence`() = runTest {
  ...
  @Test
  fun `does not swallow a rejected reopen action`() = runTest {
  ...
  @Test
  fun `checks only the selected occurrence in both widget types before its request`() {
    val presentation = mockk<CompletionPresentation>(relaxed = true)
    val targets =
      CompletionWidget.entries.mapIndexed { index, widget ->
  ...
  @Test
  fun `successful today completion refreshes and clears optimistic presentation`() = runTest {
    val presentation = mockk<CompletionPresentation>(relaxed = true)
  ...
  @Test
  fun `failed completion rolls the same control back in both widget types`() = runTest {
    val presentation = mockk<CompletionPresentation>(relaxed = true)
    val targets =
      CompletionWidget.entries.mapIndexed { index, widget ->
  ...
  @Test
  fun `completes only the selected recurring occurrence`() = runTest {
