...
class TaskWidgetRefreshTest {
  @Test
  fun `calendar week starts on Monday and ends on Sunday`() {
    assertEquals(
  ...
  @Test
  fun `only incomplete scheduled tasks inside the week are retained`() {
    val week = LocalDate.of(2026, 8, 3) to LocalDate.of(2026, 8, 9)
    val result =
      TaskWidgetRefresh.filterScheduledTasks(
        listOf(
  ...
  @Test
  fun `today range contains only the local current day`() {
    assertEquals(
  ...
  @Test
  fun `today keeps completed and incomplete tasks planned for today`() {
  ...
  @Test
  fun `next refresh is the next local midnight including Monday`() {
  ...
  @Test
  fun `week retains more than eight tasks while today keeps its capacity`() {
    val range = LocalDate.of(2026, 8, 3) to LocalDate.of(2026, 8, 9)
    val tasks =
      (0..9).map { task("task-$it", "2026-08-05T${it.toString().padStart(2, '0')}:00:00Z") }
  ...
  @Test
  fun `overdue tasks are incomplete deduplicated and ordered before today`() {
  ...
  @Test
  fun `local date determines whether a task is overdue`() {
    val today = LocalDate.of(2026, 8, 5)
    val result =
      TaskWidgetRefresh.filterScheduledTasks(
  ...
  @Test
  fun `today capacity is selected from overdue first order`() {
    val today = LocalDate.of(2026, 8, 5)
    val overdue =
      (1..8).map { day ->
        task("overdue-$day", "2026-07-${day.toString().padStart(2, '0')}T09:00:00Z")
      }
    val result =
      TaskWidgetRefresh.filterScheduledTasks(
