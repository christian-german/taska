...
public class CommentService {
  ...
  /**
   * Returns comments scoped by the provided filter. When {@code taskId} is given, only comments on
   * that task are returned. When {@code projectId} is given, only project-level comments are
   * returned. If neither is provided, all comments are returned. Results are ordered by creation
   * time ascending.
   *
  ...
  /**
   * Deletes the comment with the given ID. Throws {@link
  ...
  /**
   * Loads a comment by ID or throws {@link com.taska.exception.ResourceNotFoundException} if not
   * found.
   *
