...
public interface TaskOccurrenceNotificationRepository
    extends JpaRepository<TaskOccurrenceNotification, UUID> {

  /**
   * Atomically claims one occurrence for notification delivery.
   *
   * <p>The database uniqueness constraint on {@code (series_id, occurrence_scheduled_at)} elects a
   * single scheduler execution when runs overlap.
   *
