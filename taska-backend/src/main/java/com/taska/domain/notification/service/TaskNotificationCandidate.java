 ...
 * @param occurrenceScheduledAt stable recurring occurrence identity, or {@code null} for a
 *     non-recurring task
 */
public record TaskNotificationCandidate(
    Task task,
    String content,
    String description,
    Instant scheduledAt,
  ...
  public static TaskNotificationCandidate nonRecurring(Task task) {
    ...
    return new TaskNotificationCandidate(
        task, task.getContent(), task.getDescription(), task.getScheduledAt(), null);
  }
