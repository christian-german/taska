...
public class PlanningCalendarRulesOverlapValidator
    implements ConstraintValidator<
        NonOverlappingPlanningCalendarRules, Collection<? extends PlanningCalendarRuleValue>> {
  ...
  public boolean isValid(
    ...
    for (int dayOfWeek = 1; dayOfWeek <= 7; dayOfWeek++) {
      ...
      List<? extends PlanningCalendarRuleValue> rulesForDay =
          planningCalendarRules.stream()
              .filter(
                  rule ->
                      rule != null
                          && rule.dayOfWeek() != null
