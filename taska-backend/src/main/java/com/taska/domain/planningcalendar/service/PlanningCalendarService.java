...
public class PlanningCalendarService {
  ...
  public PlanningCalendarDetails get(@NotNull UUID planningCalendarId) {
    return toDetails(
        planningCalendarRepository
            .findById(planningCalendarId)
            .orElseThrow(
                () ->
  ...
  public PlanningCalendarDetails create(
      @NotNull @Valid PlanningCalendarCreateParameters planningCalendarCreateParameters) {
  ...
  public boolean allows(
      @NotNull UUID planningCalendarId, @NotNull Instant scheduledAt, boolean allDay) {

    ZoneId timeZone = taskaProperties.getCalendar().getTimeZone();
    ZonedDateTime scheduledDateTime = scheduledAt.atZone(timeZone);
    List<PlanningCalendarRule> calendarRules =
        planningCalendarRuleRepository.findByCalendarIdOrderByDayOfWeekAscStartMinuteAsc(
            planningCalendarId);
    if (allDay) {
    ...
    return calendarRules.stream()
        .anyMatch(
            rule ->
                rule.getDayOfWeek() == scheduledDateTime.getDayOfWeek().getValue()
                    && rule.getStartMinute() <= scheduledMinute
                    && scheduledMinute < rule.getEndMinute());
  }

  private void replaceRules(
      UUID planningCalendarId, List<PlanningCalendarRuleParameters> requestedRules) {

    planningCalendarRuleRepository.deleteByCalendarId(planningCalendarId);
    planningCalendarRuleRepository.saveAll(
  ...
  private PlanningCalendarDetails toDetails(PlanningCalendar planningCalendar) {
    ...
    return new PlanningCalendarDetails(
        ...
        planningCalendarRuleRepository
            ...
            .map(
                rule ->
                    new PlanningCalendarRuleParameters(
                        rule.getDayOfWeek(), rule.getStartMinute(), rule.getEndMinute()))
            .toList());
  }
}
