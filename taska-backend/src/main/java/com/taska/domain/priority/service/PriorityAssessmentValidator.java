...
public class PriorityAssessmentValidator {
  public void validateEnvelope(PriorityEvaluationBatchResponse response, Set<UUID> requestedIds) {
    ...
    for (var assessment : response.evaluations()) {
      if (assessment == null
          ...
          || !requestedIds.contains(assessment.taskId())
  ...
  public boolean isValid(PriorityEvaluationBatchResponse.Assessment assessment) {
    return assessment != null
        ...
        && confidence(assessment.urgencyConfidence())
        && confidence(assessment.impactConfidence())
        && confidence(assessment.riskConfidence())
        && confidence(assessment.durationConfidence())
        && nonBlank(assessment.urgencyReason())
        && nonBlank(assessment.impactReason())
        && nonBlank(assessment.riskReason())
        && nonBlank(assessment.durationReason());
  }
