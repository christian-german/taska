...
public class ProjectController {
  ...
  public ProjectDto create(@Valid @RequestBody ProjectCreateRequest projectCreateRequest) {
    ProjectCreateParameters projectCreateParameters =
        projectMapper.toParameters(projectCreateRequest);
  ...
  public ProjectDto update(
      @PathVariable UUID projectId, @Valid @RequestBody ProjectUpdateRequest projectUpdateRequest) {
    ProjectUpdateParameters projectUpdateParameters =
        projectMapper.toParameters(projectUpdateRequest);
  ...
  public void reorder(@RequestBody List<ProjectReorderRequest> reorderRequests) {
    List<ProjectReorderParameters> reorderParameters =
        reorderRequests.stream().map(projectMapper::toParameters).toList();
    projectService.reorder(reorderParameters);
  }

  /**
   * Deletes the project with the given ID. Returns HTTP 204 on success. Deleting the inbox project
