...
public interface ProjectMcpMapper {
  ...
  /**
   * Builds a full replacement from a partial tool input by falling back to the persisted project
   * for every omitted property. Two sources with a per-property fallback have no declarative
   * equivalent, so the merge is written out here rather than forced into a mapping expression.
   *
   ...
   */
  default ProjectUpdateParameters toParameters(
      ProjectMcpTools.ProjectUpdateInput projectUpdateInput, Project existingProject) {
  ...
  private static java.util.UUID resolveParentId(
      ProjectMcpTools.ProjectUpdateInput projectUpdateInput, Project existingProject) {
    if (Boolean.TRUE.equals(projectUpdateInput.clearParent())) {
      return null;
    }
    return projectUpdateInput.parentId() == null
