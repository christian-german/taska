 ...
 *
 * <p>Projects can be nested (via {@link #parentId}) to form a hierarchy. Exactly one project per
 * installation is flagged as the inbox ({@link #isInboxProject}); this project acts as the default
 * container for tasks created without an explicit project and cannot be deleted.
 */
@Entity
@Table(name = "projects")
public class Project {
  ...
  /**
   * UUID of the parent project; {@code null} for root-level projects. Used to build the project
   * hierarchy in the UI.
   */
  ...
  /**
   * Preferred task rendering mode for this project. Stored as a string enum; defaults to {@link
   * ViewStyle#LIST}.
   */
   ...
   * Tasks created without an explicit project are placed here. The inbox project cannot be deleted.
   */
