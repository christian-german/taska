...
public class ProjectService {
        ...
        if (!projectUpdateParameters.planningCalendarId().equals(project.getPlanningCalendarId())) {
            ...
            boolean hasIncompatibleTask = taskRepository
                    ...
                    .anyMatch(task -> !planningCalendarService.allows(
                            projectUpdateParameters.planningCalendarId(), task.getScheduledAt(), task.isAllDay()));
    ...
    public void reorder(List<ProjectReorderParameters> reorderParameters) {
        reorderParameters.forEach(reorderParameter ->
            projectRepository.findById(reorderParameter.projectId()).ifPresent(project -> {
