...
/**
 * Transfer object representing a task returned by the API. For recurring tasks each occurrence is
 * represented as a separate {@code TaskDto}, with occurrence-specific fields ({@code instanceId},
 * {@code occurrenceScheduledAt}, {@code isVirtual}) populated to distinguish it from a plain task.
 *
 ...
 * @param recurrenceRule iCal4j RRULE string describing the recurrence pattern (e.g. {@code
 *     "FREQ=DAILY"})
 ...
 * @param instanceId UUID of the persisted {@code TaskInstance} for this occurrence; {@code null}
 *     when the occurrence is virtual (not yet modified or completed)
 ...
 * @param isVirtual {@code true} when no {@code TaskInstance} exists for this occurrence (it has
 *     never been completed, skipped, or modified)
 * @param rruleEndsAt the instant at which the recurrence series is truncated; occurrences at or
 *     after this instant are not generated
 */
