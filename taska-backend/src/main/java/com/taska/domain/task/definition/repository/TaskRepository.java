...
public interface TaskRepository extends JpaRepository<Task, UUID> {
  ...
  @Query(
      """
            SELECT t FROM Task t WHERE t.isCompleted = false AND t.isRecurring = false AND t.type = com.taska.domain.task.definition.TaskType.TODO
            AND NOT EXISTS (SELECT e FROM TaskPriorityEvaluation e WHERE e.taskId = t.id) ORDER BY t.createdAt ASC
            """)
  List<Task> findEligibleTasksWithoutPriorityEvaluation(
  ...
  @Query(
      "SELECT t FROM Task t JOIN t.labels l WHERE l = :label AND t.isCompleted = false ORDER BY t.position ASC")
  ...
  /**
   * Returns all incomplete tasks in the given project that have a scheduled time, ordered by
   * scheduled time.
   */
  List<Task> findByProjectIdAndScheduledAtIsNotNullAndIsCompletedFalseOrderByScheduledAtAsc(
      UUID projectId);
  ...
  /**
   * Returns all incomplete, non-recurring tasks whose due date falls within [start, end). Ordered
   * by due date ascending. Used to build the occurrence list for a date range.
   */
  @Query(
      """
            ...
            """)
  List<Task> findNonRecurringTasksInPeriod(
      @Param("start") Instant start, @Param("end") Instant end);

  /**
   * Returns all non-recurring tasks whose scheduled date falls within [start, end), including
   * completed tasks. Used by date-range views that explicitly request them.
   */
  @Query(
      """
            SELECT t FROM Task t
            ...
            ORDER BY t.scheduledAt ASC
            """)
  List<Task> findNonRecurringTasksIncludingCompletedInPeriod(
      @Param("start") Instant start, @Param("end") Instant end);

  /**
   * Returns all recurring tasks whose series overlaps the given period. A task is included when its
   * first due date is before the period end and its {@code rruleEndsAt} (if set) has not yet passed
   * the period start.
   */
  @Query(
      """
