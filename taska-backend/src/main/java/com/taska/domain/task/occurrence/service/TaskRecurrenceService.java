...
public class TaskRecurrenceService {
  ...
  /**
   * Expands the RRULE of a recurring task and returns all occurrence instants that fall within the
   * given period. Returns an empty list when the task has no due date or when the series has
   * already ended before {@code periodStart}.
   *
   * <p>Occurrences are post-filtered against {@code rruleEndsAt}: any occurrence on or after that
  ...
  public List<Instant> getOccurrencesInRange(Task task, Instant periodStart, Instant periodEnd) {
    ...
    if (!Boolean.TRUE.equals(task.getIsRecurring()) || task.getRecurrenceRule() == null) {
      throw new IllegalStateException(
          "Task " + task.getId() + " is_recurring=false or has no recurrence_rule");
    }
