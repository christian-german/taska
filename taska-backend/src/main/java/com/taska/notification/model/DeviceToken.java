 ...
 *
 * <p>
 * Tokens are registered (or refreshed) when the mobile app calls the device-registration endpoint. The registration performs an upsert: if a row with
 * the same {@link #token} value already exists (looked up via {@link DeviceTokenRepository#findByToken}), its timestamps are updated; otherwise a new
 * row is inserted. The token column is capped at 512 characters to accommodate FCM token lengths.
 */
@Entity
