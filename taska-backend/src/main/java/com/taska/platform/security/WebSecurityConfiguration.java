package com.taska.platform.security;
...
public class WebSecurityConfiguration {
    ...
    /**
     * Configures the main security filter chain. CSRF is disabled (stateless API), CORS is enabled with default settings, sessions are never created,
     * and all endpoints require an authenticated OAuth2 JWT except the actuator health check.
     *
    ...
    /**
     * Configures CORS to allow all origins, methods, and headers on every endpoint. This is intentionally permissive; restrict in production if the
     * API is not intended to be publicly accessible from any origin.
     *
    ...
    /**
     * Configures and returns a {@link JwtDecoder} bean for decoding JWT tokens. The decoder is initialized using the issuer URI specified in the
     * application properties and optionally customizes the JWT validation logic based on configuration. Additionally, HTTP connection and read
