...
public class TaskPriorityEvaluationService {
    ...
    private final PriorityAssessmentClient priorityAssessmentClient;
    ...
    public void evaluate(List<Task> tasks) {
        ...
        Map<UUID, Instant> taskVersions = tasks.stream().collect(Collectors.toMap(Task::getId, Task::getUpdatedAt));
        Set<UUID> taskIds = taskVersions.keySet();
        PriorityAssessmentBatchResult evaluationResponse = priorityAssessmentClient.assess(PriorityAssessmentBatch.from(tasks));
        ...
        for (var assessment : evaluationResponse.evaluations()) {
            ...
            evaluation.setScore(taskPriorityScorer.total(assessment.urgency(), assessment.impact(), assessment.risk(), assessment.durationMinutes()));
    ...
    private JsonNode components(PriorityAssessmentBatchResult.Assessment assessment) {
