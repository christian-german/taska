...
public class ProjectController {
    ...
    public void reorder(@RequestBody List<ProjectReorderRequest> reorderRequests) {
        List<ProjectReorderParameters> reorderParameters = reorderRequests.stream().map(projectMapper::toParameters).toList();
        projectService.reorder(reorderParameters);
    }

    /**
     * Deletes the project with the given ID. Returns HTTP 204 on success. Deleting the inbox project is not allowed and will result in an error.
     *
