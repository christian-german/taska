...
public interface TaskMcpMapper {
    ...
    default TaskMcpOutput toOutput(Task task) {
        return Boolean.TRUE.equals(task.getIsRecurring()) ? toRecurringSeriesOutput(task) : toStoredTaskOutput(task);
    }
    ...
    /**
     * Flattens one recurring occurrence for the MCP schema. The effective values come from the result, which owns the override rule shared with the
     * HTTP adapter; only the exposed shape is MCP-specific.
     *
