...
public class TaskMcpTools {
    ...
    public McpSchema.CallToolResult listTasks(
            @McpToolParam(description = "Optional filters. Use null for filters not needed.") TaskListInput taskListInput) {
        return McpToolResponses.execute(
                () -> new TaskListOutput(
                        taskService.findAll(taskListInput.projectId(), taskListInput.label(), taskListInput.showCompleted())
                                .stream()
    ...
    public McpSchema.CallToolResult createTask(@McpToolParam(description = "New task details.") TaskCreateInput taskCreateInput) {
        return McpToolResponses.execute(() -> {
            ...
            return taskMcpMapper.toOutput(taskMutationService.create(taskMcpMapper.toParameters(taskCreateInput), accountSubject()));
        });
    }

    @McpTool(name = "update_task", description = "Update task fields. Existing series schedules and recurrence rules are immutable. To move one occurrence, provide scope THIS_ONLY, occurrenceScheduledAt and scheduledAt only. FROM_THIS edits are not supported. Set clearPriority to true for a base task's manual priority.", generateOutputSchema = true)
    public McpSchema.CallToolResult updateTask(
        ...
        return McpToolResponses.execute(() -> {
            ...
            boolean priorityProvided = taskUpdateInput.priority() != null || Boolean.TRUE.equals(taskUpdateInput.clearPriority());
            return taskMcpMapper
                    .toOutput(taskMutationService.update(taskId, taskMcpMapper.toParameters(taskUpdateInput), priorityProvided, accountSubject()));
        });
    }
    ...
    public McpSchema.CallToolResult completeTask(
        ...
        return McpToolResponses.execute(
                () -> taskMcpMapper
                        .toOutput(taskMutationService.close(taskId, new TaskCloseReopenParameters(occurrenceScheduledAt), accountSubject())));
    }
    ...
    public McpSchema.CallToolResult reopenTask(
        ...
        return McpToolResponses.execute(
                () -> taskMcpMapper
                        .toOutput(taskMutationService.reopen(taskId, new TaskCloseReopenParameters(occurrenceScheduledAt), accountSubject())));
    }
    ...
    public record TaskListInput(@McpToolParam(required = false) UUID projectId, @McpToolParam(required = false) String label,
    ...
    public record TaskCreateInput(@McpToolParam(description = "Task title.") String content,
    ...
    public record TaskUpdateInput(@McpToolParam(required = false, description = "Replacement task title.") String content,
            ...
            @McpToolParam(required = false, description = "THIS_ONLY to move one occurrence; FROM_THIS edits are not supported.") RecurrenceScope scope,
