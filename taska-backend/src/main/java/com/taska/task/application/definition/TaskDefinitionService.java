...
public class TaskDefinitionService {
    ...
    /**
     * Returns the incomplete tasks of one project, ordered by their position.
     *
    ...
    /**
     * Returns the task with the given ID, or throws {@link com.taska.platform.exception.ResourceNotFoundException} if no such task exists.
     *
    ...
    /**
     * Creates and persists a new task from the given request. If no {@code projectId} is provided and the task has no parent, it is placed in the
     * inbox project. Defaults: position 0, not recurring, and not all-day. Manual priority remains absent when the request does not provide one. The
    ...
    public Task create(TaskCreateParameters taskCreateParameters) {
        TaskDefinitionRules.assertDueAtAllowed(taskCreateParameters.recurring(), taskCreateParameters.dueAt());
        String createdRecurrenceRule = TaskDefinitionRules.normalizeRecurrenceRule(taskCreateParameters.recurrenceRule());
        TaskDefinitionRules.assertRecurrenceRuleRequired(taskCreateParameters.recurring(), createdRecurrenceRule);
        TaskDefinitionRules.assertScheduledAtRequired(taskCreateParameters.recurring(), taskCreateParameters.scheduledAt());
        Task task = new Task();
        ...
        task.setLabels(taskCreateParameters.labels() != null ? taskCreateParameters.labels() : new ArrayList<>());
        ...
        if (projectId == null && taskCreateParameters.parentId() == null) {
            projectId = projectService.inboxProjectId();
        }
     ...
     *
     * <p>
     * {@code priorityProvided} retains JSON field-presence information: it distinguishes an omitted priority from an explicit JSON null, the former
     * leaving manual priority unchanged and the latter clearing it.
     *
    ...
    public TaskResult reopenTask(UUID taskId) {
        Task task = getOrThrow(taskId);
        if (Boolean.TRUE.equals(task.getIsRecurring())) {
            throw new IllegalArgumentException("occurrenceScheduledAt is required to reopen a recurring occurrence");
        }
    ...
    /**
     * Loads a task by ID or throws {@link com.taska.platform.exception.ResourceNotFoundException} if not found.
     *
     * @param taskId the task UUID
     * @return the task entity
     */
    private Task getOrThrow(UUID taskId) {
        return taskRepository.findById(taskId).orElseThrow(() -> new ResourceNotFoundException("Task not found: " + taskId));
    }

    /**
     * Returns all incomplete, non-notified, non-all-day tasks whose due date is at or before the given instant. Used by the notification scheduler to
     * find tasks due in approximately 15 minutes.
     *
    ...
    /**
     * Applies non-null fields from the request onto an existing task entity. When the planned scheduled time changes, the notification flag is reset
     * so the task can trigger a new push notification at its new time.
     *
     ...
     * @param priorityProvided whether the caller explicitly supplied the priority field
     */
    private void applyPatch(Task task, TaskPatchParameters taskPatchParameters, boolean priorityProvided) {
        boolean recurring = taskPatchParameters.recurring() != null ? taskPatchParameters.recurring() : Boolean.TRUE.equals(task.getIsRecurring());
        TaskDefinitionRules.assertDueAtAllowed(recurring, taskPatchParameters.dueAt());
        String patchedRecurrenceRule = taskPatchParameters.recurrenceRule() != null
        ...
        Instant patchedScheduledAt = taskPatchParameters.scheduledAt() != null ? taskPatchParameters.scheduledAt() : task.getScheduledAt();
        ...
        if (taskPatchParameters.recurrenceRule() != null) {
            task.setRecurrenceRule(TaskDefinitionRules.normalizeRecurrenceRule(taskPatchParameters.recurrenceRule()));
        }
        if (recurring) {
            // Converting a task to a series removes an absolute deadline inherited from the
            // former task.
            task.setDueAt(null);
        ...
        if (taskPatchParameters.scheduledAt() != null) {
            UUID effectiveProjectId = taskPatchParameters.projectId() != null ? taskPatchParameters.projectId() : task.getProjectId();
            boolean effectiveAllDay = taskPatchParameters.allDay() != null ? taskPatchParameters.allDay() : task.isAllDay();
    ...
    private void replaceMutableFields(Task task, TaskUpdateParameters taskUpdateParameters) {
        ...
        TaskDefinitionRules.assertDueAtAllowed(taskUpdateParameters.recurring(), taskUpdateParameters.dueAt());
        String replacementRecurrenceRule = TaskDefinitionRules.normalizeRecurrenceRule(taskUpdateParameters.recurrenceRule());
        TaskDefinitionRules.assertRecurrenceRuleRequired(taskUpdateParameters.recurring(), replacementRecurrenceRule);
        TaskDefinitionRules.assertScheduledAtRequired(taskUpdateParameters.recurring(), taskUpdateParameters.scheduledAt());
        ...
        assertScheduleAllowed(taskUpdateParameters.projectId(), taskUpdateParameters.scheduledAt(), taskUpdateParameters.allDay());
    }
    ...
    public void assertScheduleAllowed(UUID projectId, Instant scheduledAt, boolean allDay) {
        ...
        UUID calendarId = projectService.planningCalendarIdOf(projectId);
