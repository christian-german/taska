package com.taska.task.application.recurrence;
...
public class TaskRecurrenceService {
    ...
    /**
     * Expands the RRULE of a recurring task and returns all occurrence instants that fall within the given period. Returns an empty list when the
     * task has no due date or when the series has already ended before {@code periodStart}.
     *
     * <p>
     * Occurrences are post-filtered against {@code rruleEndsAt}: any occurrence on or after that cutoff is excluded, regardless of what iCal4j
     * returns.
     *
     * @param task the recurring task — must have {@code isRecurring = true} and a non-null {@code
     *     recurrenceRule}, otherwise an {@link IllegalStateException} is thrown
    ...
    public List<Instant> getOccurrencesInRange(Task task, Instant periodStart, Instant periodEnd) {
        ...
        try {
            ...
            throw new IllegalArgumentException("Invalid recurrence_rule for task " + task.getId() + ": " + task.getRecurrenceRule(), exception);
        }
