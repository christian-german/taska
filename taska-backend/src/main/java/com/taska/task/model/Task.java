 ...
 *
 * <p>
 * A task can be a top-level item, a subtask (via {@code parentId}), or a recurring series. For recurring tasks, sparse occurrence state is tracked
 * separately by the occurrence subfeature. Labels are stored in a join table ({@code task_labels}) and loaded eagerly.
 */
@Entity
@Table(name = "tasks")
public class Task {
    ...
    /**
     * Denormalised reference to the owning project. {@code null} for subtasks that inherit their project from the parent.
     */
    ...
    /**
     * Optional urgency level: 1 = urgent, 2 = high, 3 = medium, 4 = normal. A {@code null} value means no manual priority has been assigned.
     */
    @Column
    private Integer priority;

    /**
     * Label names attached to this task. Persisted in the {@code task_labels} join table and always fetched eagerly to avoid lazy-loading issues
    ...
    /**
     * Raw context captured when the task was created via an @-mention (used by mobile clients). Max 100 characters.
     */
    @Column(name = "mention_context", length = 100)
    private String mentionContext;

    /**
     * iCal4j RRULE string defining the recurrence pattern (e.g. {@code "FREQ=DAILY"}). Only meaningful when {@link #isRecurring} is {@code true}. Max
     * 100 characters.
     */
    ...
    /**
     * Instant at which the recurrence series is truncated. Occurrences at or after this instant are not generated. Set by the {@code FROM_THIS}
     * series-stopping deletion operation.
     */
    ...
    /**
     * Guards against duplicate push notifications. Set to {@code true} after a notification is sent; reset to {@code false} whenever
     * {@link #scheduledAt} is changed so the task can fire a new notification.
     */
