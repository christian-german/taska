...
/**
 * JPA entity representing sparse persisted state for a recurring {@link Task} occurrence in the {@code task_occurrence_states} table.
 *
 * <p>
 * Most occurrences of a recurring task are <em>virtual</em> — generated on the fly from the task's RRULE — and have no corresponding row here. A
 * {@code TaskOccurrenceState} is created only when an occurrence is explicitly acted on: completed ({@code DONE}), skipped ({@code SKIPPED}), or
 * modified ({@code MODIFIED}). The combination of {@link #seriesId} and {@link #occurrenceScheduledAt} is effectively a unique key identifying a
 * specific occurrence.
 */
@Entity
...
public class TaskOccurrenceState {
    ...
    /**
     * The RRULE-generated instant this state corresponds to. Together with {@link #seriesId}, this uniquely identifies a single occurrence.
     */
    @Column(name = "occurrence_scheduled_at", nullable = false)
    private Instant occurrenceScheduledAt;

    /**
     * Override for the occurrence's planned schedule time; non-null only when the occurrence was modified via a {@code THIS_ONLY} update that changed
     * the task's {@code scheduledAt}.
     */
    ...
    /**
     * Whether the series no longer generates {@link #occurrenceScheduledAt}. A detached state stands on its own: it is still displayed, still belongs
     * to its series, but overlays nothing.
     */
    @Column(nullable = false)
    ...
    /**
     * Timestamp when this state row was first persisted; set by {@link #onCreate()} and never updated.
     */
    @Column(name = "created_at", nullable = false, updatable = false)
    private Instant createdAt;

    /**
     * Timestamp of the last modification to this occurrence-state row; updated automatically by {@link #onUpdate()}.
     */
