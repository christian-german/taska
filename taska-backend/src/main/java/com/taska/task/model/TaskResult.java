 ...
 */
public sealed interface TaskResult permits NonRecurringTaskResult, RecurringTaskOccurrenceResult, RecurringTaskSeriesResult {

    Task task();

    /**
     * Creates a task result from a task. The task result can be a non-recurring task result or a recurring task series result.
     *
    ...
    static TaskResult base(Task task) {
        ...
        return Boolean.TRUE.equals(task.getIsRecurring()) ? new RecurringTaskSeriesResult(task) : new NonRecurringTaskResult(task);
    }
     ...
     * @return a task result
     */
    static TaskResult occurrence(Task task, TaskOccurrenceState occurrenceState, Instant occurrenceScheduledAt) {
        Objects.requireNonNull(task, "task");
