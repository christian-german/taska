...
class PlanningCalendarServiceTest {
  ...
  void allowsAnInstantWithinItsWeeklyRule() {
    ...
    assertThat(planningCalendarService.allows(id, Instant.parse("2026-05-18T08:00:00Z"), false))
        .isTrue();
    assertThat(planningCalendarService.allows(id, Instant.parse("2026-05-18T16:00:00Z"), false))
        .isFalse();
  }

  @Test
  void validatesOverlappingRulesAtTheServiceBoundary() throws NoSuchMethodException {
    PlanningCalendarCreateParameters planningCalendarParameters =
    ...
    Method createMethod =
        PlanningCalendarService.class.getMethod("create", PlanningCalendarCreateParameters.class);

    Set<ConstraintViolation<PlanningCalendarService>> violations =
        validator
            .forExecutables()
            .validateParameters(
                planningCalendarService, createMethod, new Object[] {planningCalendarParameters});
  ...
  void updateWithAnEmptyRuleListClearsExistingRules() {
    ...
    var details =
        planningCalendarService.update(
