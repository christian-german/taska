...
class TaskMcpToolsTest {
  ...
  void listTasksUsesAnObjectAsStructuredContentRoot() {
    ...
    McpSchema.CallToolResult callToolResult =
        taskMcpTools.listTasks(new TaskMcpTools.TaskListInput(null, null, false));
  ...
  void createTaskWithoutScheduleOrPriorityPreservesInboxAndNullableFields() {
    ...
    ArgumentCaptor<TaskCreateParameters> taskRequestCaptor =
        ArgumentCaptor.forClass(TaskCreateParameters.class);
  ...
  void completeTaskPassesRecurringOccurrenceTimestampToTaskService() {
    ...
    McpSchema.CallToolResult callToolResult =
        taskMcpTools.completeTask(taskId, occurrenceScheduledAt);
  ...
  void invalidPriorityIsReturnedAsSafeToolError() {
    ...
    assertThat(callToolResult.content().getFirst().toString())
