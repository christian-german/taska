...
class TaskNotificationCandidateServiceTest {
    ...
    @InjectMocks
    private TaskNotificationCandidateService service;
    ...
    void virtualOccurrenceInTheWindowIsEligibleWithoutMaterializingIt() {
        ...
        when(taskRepository.findActiveRecurringTasksForPeriod(WINDOW_START, WINDOW_END)).thenReturn(List.of(task));
        when(taskOccurrenceStateRepository.findByStatusAndScheduledAtBetween(TaskOccurrenceStatus.MODIFIED, WINDOW_START, WINDOW_END))
                .thenReturn(List.of());
        when(taskOccurrenceStateRepository.findBySeriesIdInAndOccurrenceScheduledAtBetween(List.of(task.getId()), WINDOW_START, WINDOW_END))
                .thenReturn(List.of());
        when(taskRecurrenceService.getOccurrencesInRange(task, WINDOW_START, WINDOW_END)).thenReturn(List.of(occurrenceScheduledAt));

        assertThat(service.findEligible(WINDOW_START, WINDOW_END)).singleElement().satisfies(candidate -> {
    ...
    void modifiedOccurrenceMovedIntoTheWindowUsesItsEffectiveValues() {
        ...
        TaskOccurrenceState occurrenceState = occurrenceState(task, occurrenceScheduledAt, TaskOccurrenceStatus.MODIFIED);
        occurrenceState.setTitle("Moved occurrence");
        occurrenceState.setScheduledAt(movedScheduledAt);
        when(taskRepository.findActiveRecurringTasksForPeriod(WINDOW_START, WINDOW_END)).thenReturn(List.of());
        when(taskRepository.findAllById(Set.of(task.getId()))).thenReturn(List.of(task));
        when(taskOccurrenceStateRepository.findByStatusAndScheduledAtBetween(TaskOccurrenceStatus.MODIFIED, WINDOW_START, WINDOW_END))
                .thenReturn(List.of(occurrenceState));
        when(taskOccurrenceStateRepository.findBySeriesIdInAndOccurrenceScheduledAtBetween(List.of(task.getId()), WINDOW_START, WINDOW_END))
                .thenReturn(List.of());
        when(taskRecurrenceService.getOccurrencesInRange(task, WINDOW_START, WINDOW_END)).thenReturn(List.of());
    ...
    void completedAndSkippedOccurrencesAreNotEligible() {
        ...
        when(taskRecurrenceService.getOccurrencesInRange(task, WINDOW_START, WINDOW_END)).thenReturn(List.of(completedAt, skippedAt));
    ...
    private TaskOccurrenceState occurrenceState(Task task, Instant occurrenceScheduledAt, TaskOccurrenceStatus status) {
    ...
    void aSeriesWithAnUnusableRuleDoesNotCostTheOtherTasksTheirNotification() {
        ...
        Instant occurrenceScheduledAt = Instant.parse("2026-05-20T10:10:00Z");
        when(taskRepository.findActiveRecurringTasksForPeriod(WINDOW_START, WINDOW_END)).thenReturn(List.of(broken, healthy));
        when(taskOccurrenceStateRepository.findByStatusAndScheduledAtBetween(TaskOccurrenceStatus.MODIFIED, WINDOW_START, WINDOW_END))
                .thenReturn(List.of());
        when(
                taskOccurrenceStateRepository
                        .findBySeriesIdInAndOccurrenceScheduledAtBetween(List.of(broken.getId(), healthy.getId()), WINDOW_START, WINDOW_END))
                .thenReturn(List.of());
        ...
        when(taskRecurrenceService.getOccurrencesInRange(healthy, WINDOW_START, WINDOW_END)).thenReturn(List.of(occurrenceScheduledAt));
