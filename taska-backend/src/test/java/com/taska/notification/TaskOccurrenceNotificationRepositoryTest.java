...
import com.taska.notification.persistence.TaskOccurrenceNotificationRepository;
...
class TaskOccurrenceNotificationRepositoryTest {
    ...
    @Autowired
    private TaskOccurrenceStateRepository occurrenceStateRepository;
    @Autowired
    private TaskOccurrenceNotificationRepository repository;
    @Autowired
    ...
    void migrationUsesOccurrenceStateAndSeriesVocabulary() {
        ...
        assertThat(occurrenceStateRepository.findBySeriesIdAndOccurrenceScheduledAt(series.getId(), occurrenceScheduledAt)).contains(state);
    }
    ...
    private int claim(UUID seriesId, Instant occurrenceScheduledAt) {
        return repository.claim(UUID.randomUUID(), seriesId, occurrenceScheduledAt, Instant.parse("2026-05-20T09:45:00Z"));
    }
}
