...
import com.taska.planningcalendar.adapter.http.PlanningCalendarCreateRequest;
...
class PlanningCalendarValidationTest {
    ...
    void createRequestValidatesNestedRulePresenceAndBounds() {
        PlanningCalendarCreateRequest request = new PlanningCalendarCreateRequest(
                "Work",
                List.of(new PlanningCalendarCreateRequest.Rule(8, null, 1_441)));
    ...
    void updateRequestValidatesRuleRangeAndOverlap() {
        PlanningCalendarUpdateRequest request = new PlanningCalendarUpdateRequest(
                "Work",
                List.of(new PlanningCalendarUpdateRequest.Rule(1, 540, 720), new PlanningCalendarUpdateRequest.Rule(1, 600, 600)));
    ...
    void createParametersIndependentlyValidateTheirContract() {
        PlanningCalendarCreateParameters parameters = new PlanningCalendarCreateParameters(
                " ",
                List.of(new PlanningCalendarRuleParameters(0, 720, 540)));

        assertThat(validator.validate(parameters)).extracting(violation -> violation.getPropertyPath().toString())
                .contains("name", "rules[0].dayOfWeek", "rules[0]");
    }

    @Test
    void updateParametersRequireRulesAndAllowTouchingWindows() {
        PlanningCalendarUpdateParameters missingRules = new PlanningCalendarUpdateParameters("Work", null);
        PlanningCalendarUpdateParameters touchingRules = new PlanningCalendarUpdateParameters(
                "Work",
                List.of(new PlanningCalendarRuleParameters(1, 540, 720), new PlanningCalendarRuleParameters(1, 720, 780)));

        assertThat(validator.validate(missingRules)).extracting(violation -> violation.getPropertyPath().toString()).containsExactly("rules");
