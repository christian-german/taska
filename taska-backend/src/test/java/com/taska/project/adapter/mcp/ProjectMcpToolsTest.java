...
class ProjectMcpToolsTest {
    ...
    @BeforeEach
    void createTools() {
        projectMcpTools = new ProjectMcpTools(projectService, new ProjectMapperImpl(), new ProjectMcpMapperImpl());
    }
    ...
    void listProjectsDelegatesToProjectService() {
        ...
        assertThat(callToolResult.structuredContent()).isEqualTo(new ProjectMcpTools.ProjectListOutput(List.of()));
        verify(projectService).findAll();
    }

    @Test
    void createProjectRejectsBlankNameWithoutCallingService() {
        McpSchema.CallToolResult callToolResult = projectMcpTools
                .createProject(new ProjectMcpTools.ProjectCreateInput(" ", null, null, null, null, null));
