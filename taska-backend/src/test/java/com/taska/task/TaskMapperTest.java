...
import com.taska.task.adapter.http.NonRecurringTaskDto;
import com.taska.task.adapter.http.RecurringTaskOccurrenceDto;
import com.taska.task.adapter.http.RecurringTaskSeriesDto;
import com.taska.task.adapter.http.TaskMapper;
...
/**
 * Tests the default method TaskMapper#toOccurrenceDto, which merges a recurring Task with an optional TaskOccurrenceState (field-level inheritance
 * logic).
 */
class TaskMapperTest {
    ...
    void toDto_nonRecurringTask_returnsNonRecurringRepresentation() {
        ...
        assertThat(taskMapper.toDto(task)).isInstanceOf(NonRecurringTaskDto.class).extracting("kind").isEqualTo(TaskRepresentationKind.NON_RECURRING);
    }
    ...
    void toDto_recurringTask_returnsSeriesRepresentation() {
        ...
        assertThat(taskMapper.toDto(task)).isInstanceOf(RecurringTaskSeriesDto.class)
                .extracting("kind")
    ...
    void toOccurrenceDto_noInstance_isVirtualTrueInstanceIdNull() {
        ...
        RecurringTaskOccurrenceDto taskDto = taskMapper.toOccurrenceDto(new RecurringTaskOccurrenceResult(task, null, occurrenceScheduledAt));
    ...
    void toOccurrenceDto_doneInstance_isCompletedTrueIsVirtualFalse() {
        Task task = buildTask("My task", 2);
        Instant occurrenceScheduledAt = Instant.parse("2026-05-20T10:00:00Z");
        TaskOccurrenceState instance = buildOccurrenceState(task.getId(), occurrenceScheduledAt, TaskOccurrenceStatus.DONE);
        instance.setCompletedAt(Instant.parse("2026-05-20T11:00:00Z"));

        RecurringTaskOccurrenceDto taskDto = taskMapper.toOccurrenceDto(new RecurringTaskOccurrenceResult(task, instance, occurrenceScheduledAt));
    ...
    void toOccurrenceDto_modifiedInstanceWithTitleAndPriority_usesInstanceValues() {
        ...
        Instant occurrenceScheduledAt = Instant.parse("2026-05-20T10:00:00Z");
        TaskOccurrenceState instance = buildOccurrenceState(task.getId(), occurrenceScheduledAt, TaskOccurrenceStatus.MODIFIED);
    ...
    void toOccurrenceDto_dueAtComesOnlyFromOccurrenceOverride() {
        ...
        assertThat(taskMapper.toOccurrenceDto(new RecurringTaskOccurrenceResult(task, null, occurrenceScheduledAt)).dueAt()).isNull();

        TaskOccurrenceState instance = buildOccurrenceState(task.getId(), occurrenceScheduledAt, TaskOccurrenceStatus.MODIFIED);
        instance.setDueAt(overrideDueAt);
        assertThat(taskMapper.toOccurrenceDto(new RecurringTaskOccurrenceResult(task, instance, occurrenceScheduledAt)).dueAt())
    ...
    void toOccurrenceDto_modifiedInstanceAllOverrideFieldsNull_allInheritedFromParent() {
        ...
        Instant occurrenceScheduledAt = Instant.parse("2026-05-20T10:00:00Z");
        TaskOccurrenceState instance = buildOccurrenceState(task.getId(), occurrenceScheduledAt, TaskOccurrenceStatus.MODIFIED);
        // title, priority and scheduledAt are all null — every field falls back to the
        // parent
        ...
        assertThat(taskDto.scheduledAt()).isEqualTo(occurrenceScheduledAt); // falls back to occurrenceScheduledAt when no scheduledAt
        // override
    }
    ...
    void toOccurrenceDto_missingLegacyType_defaultsToTodo() {
        ...
        assertThat(taskMapper.toOccurrenceDto(new RecurringTaskOccurrenceResult(task, null, Instant.parse("2026-05-20T10:00:00Z"))).type())
    ...
    void toOccurrenceDto_detachedState_isReportedAsDetached() {
        ...
        Instant occurrenceScheduledAt = Instant.parse("2026-05-20T10:00:00Z");
        TaskOccurrenceState detached = buildOccurrenceState(task.getId(), occurrenceScheduledAt, TaskOccurrenceStatus.DONE);
        detached.setDetached(true);

        assertThat(taskMapper.toOccurrenceDto(new RecurringTaskOccurrenceResult(task, detached, occurrenceScheduledAt)).isDetached()).isTrue();
    }
    ...
    void toOccurrenceDto_generatedOccurrence_isNotReportedAsDetached() {
        ...
        assertThat(taskMapper.toOccurrenceDto(new RecurringTaskOccurrenceResult(task, null, occurrenceScheduledAt)).isDetached()).isFalse();
    }
}
