  ...
  template: `
    <div class="modal-veil" (click)="close.emit()">
      ...
      >
        ...
        <div
          class="mono"
          style="font-size: 12px; color: var(--mute); margin-bottom: 18px; line-height: 1.5;"
        >
        ...
        @if (parsedTasks().length && !importing() && !done()) {
          <div style="margin-bottom: 14px;">
            <div style="font-size: 13px; color: var(--ink); margin-bottom: 10px; font-weight: 500;">
              {{ parsedTasks().length }} tâche{{ parsedTasks().length > 1 ? 's' : '' }} prête{{
            ...
            >
              @for (t of previewTasks(); track $index) {
                ...
                >
                  <span
                    class="mono"
                    style="font-size: 10px; color: var(--mute); flex-shrink: 0; margin-top: 2px;"
                  >
                    P{{ t.priority }}
                  </span>
                  <div style="flex: 1; min-width: 0;">
                    <div
                      style="color: var(--ink); white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"
                    >
                      {{ t.content }}
                    </div>
                    @if (t.scheduledAt) {
                      <div
                        class="mono"
                        style="font-size: 10.5px; color: var(--mute); margin-top: 2px;"
                      >
        ...
        @if (importing()) {
          <div style="margin-bottom: 14px;">
            ...
            <div
              style="height: 6px; background: var(--bg-2); border-radius: 3px; overflow: hidden;"
            >
              <div
                style="height: 100%; background: var(--accent); border-radius: 3px; transition: width .2s;"
                [style.width.%]="(importedCount() / parsedTasks().length) * 100"
              ></div>
            </div>
          </div>
        }
        ...
        @if (done()) {
          <div
            style="display: flex; align-items: center; gap: 10px; padding: 14px; background: var(--bg-2);
                      border-radius: 8px; color: var(--ink); font-size: 13px; margin-bottom: 14px;"
          >
            <app-icon name="check" [size]="16" color="var(--accent)" />
            {{ parsedTasks().length }} tâche{{ parsedTasks().length > 1 ? 's' : '' }} importée{{
              parsedTasks().length > 1 ? 's' : ''
            }}
            avec succès.
          </div>
