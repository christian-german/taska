...
@Component({
  ...
  template: `
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">
      ...
      <span
        class="mono"
        style="font-size:12px;font-weight:600;text-transform:capitalize;color:var(--ink);"
      >
        {{ monthLabel() }}
      </span>
    ...
    <div style="display:grid;grid-template-columns:repeat(7,1fr);margin-bottom:4px;">
      @for (h of DAY_HEADERS; track $index) {
        <div class="mono" style="text-align:center;font-size:10px;color:var(--mute);padding:2px 0;">
          {{ h }}
        </div>
      }
    </div>
    ...
    @if (withTime()) {
      <div style="margin-top:10px;padding-top:10px;border-top:1px solid var(--line-2);">
        ...
        <div style="display:flex;align-items:center;gap:6px;">
          <select
            [value]="selectedHour()"
            (change)="onHourChange($any($event.target).value)"
