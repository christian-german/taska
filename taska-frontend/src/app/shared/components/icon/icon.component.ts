...
const ICON_PATHS: Record<string, string> = {
  ...
  sparkle: '<path d="M9.94 14.5 8 22l-1.94-7.5L1 12l5.06-2.5L8 2l1.94 7.5L15 12l-5.06 2.5z"/><path d="M19 4l1 4 4 1-4 1-1 4-1-4-4-1 4-1z"/>',
...
@Component({
  selector: 'app-icon',
  template: `<span class="ic-wrap" [style.width.px]="size()" [style.height.px]="size()" [style.color]="color() || 'currentColor'" [innerHTML]="safeSvg()"></span>`,
  styles: [`
    .ic-wrap {
      display: inline-flex;
      align-items: center;
...
export class IconComponent {
  ...
  safeSvg = computed<SafeHtml>(() => {
    ...
    const svg = `<svg width="${this.size()}" height="${this.size()}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">${inner}</svg>`;
