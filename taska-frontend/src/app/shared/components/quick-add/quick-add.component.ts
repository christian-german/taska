      ...
      <div class="modal" (click)="$event.stopPropagation()">
        <div style="padding: 20px 22px 12px;">
          ...
          @if (detected().length > 0) {
            <div
              style="margin-top:14px;display:flex;flex-wrap:wrap;gap:6px;
          ...
          @if (activePicker()) {
            <div style="position:fixed;inset:0;z-index:55;" (click)="activePicker.set(null)"></div>
          }

          <div style="display:flex;flex-direction:column;gap:2px;color:var(--ink-2);">
            <div
              style="display:flex;gap:6px;padding:6px 8px;"
              role="group"
            ...
            >
              @for (option of taskTypes; track option.value) {
                <button
                  type="button"
                  (click)="taskType.set(option.value)"
            ...
            <div style="position:relative;">
                ...
                >
                  @if (effectiveProject() && !effectiveProject()!.isInboxProject) {
                    <span
                      ...
                      [style.background]="getColor(effectiveProject()!.color)"
                    ></span>
                  }
                  {{
                    effectiveProject()?.isInboxProject
                      ? 'Inbox'
                      : (effectiveProject()?.name ?? 'Inbox')
                  }}
                </span>
                @if (manualProjectId()) {
                  <span
                    (click)="clearManual($event, 'project')"
                    style="color:var(--mute);font-size:15px;line-height:1;cursor:pointer;"
                    >×</span
                  >
                }
              ...
              @if (activePicker() === 'project') {
                <div
                  style="position:absolute;left:0;top:calc(100% + 4px);z-index:56;background:var(--bg);border:1px solid var(--line);border-radius:10px;box-shadow:0 6px 24px rgba(0,0,0,.14);padding:8px;min-width:220px;max-height:260px;"
                  (click)="$event.stopPropagation()"
                >
                  <input
                  ...
                  <div style="overflow-y:auto;max-height:180px;">
                    ...
                    <button
                      ...
                      [style.background]="
                        !manualProjectId() ? 'rgba(255,138,61,0.08)' : 'transparent'
                      "
                      [style.color]="!manualProjectId() ? 'var(--orange)' : 'var(--ink)'"
                    >
                      <span
                        style="width:8px;height:8px;border-radius:50%;background:var(--mute);flex-shrink:0;"
                      ></span>
                      Inbox
                    </button>
            ...
            <div style="position:relative;">
              ...
              @if (activePicker() === 'tags') {
                <div
                  style="position:absolute;left:0;top:calc(100% + 4px);z-index:56;background:var(--bg);border:1px solid var(--line);border-radius:10px;box-shadow:0 6px 24px rgba(0,0,0,.14);padding:10px;min-width:260px;"
                  (click)="$event.stopPropagation()"
                >
                  <input
                    [ngModel]="tagSearch()"
                  ...
                  <!-- Existing labels -->
                  <div
                    style="display:flex;flex-wrap:wrap;gap:5px;max-height:130px;overflow-y:auto;"
                  >
                    @for (label of filteredLabels(); track label.id) {
                      <button
                        ...
                        style="display:flex;align-items:center;gap:4px;padding:3px 8px;
                                     ...
                                     transition:all .12s;"
                        [style.border-color]="
                          rgba(getColor(label.color), effectiveTagSet().has(label.name) ? 1 : 0.3)
                        "
                        [style.background]="
                          effectiveTagSet().has(label.name)
                            ? rgba(getColor(label.color), 0.15)
                            : 'transparent'
                        "
                        [style.color]="
                          effectiveTagSet().has(label.name) ? getColor(label.color) : 'var(--ink-2)'
                        "
                      >
                  ...
                  @if (effectiveTags().length > 0) {
                    ...
                    >
                      <span
                        class="mono"
                        style="color:var(--mute);font-size:10.5px;width:100%;margin-bottom:2px;"
                      >
                        sélectionnés :
                      </span>
                      @for (tag of effectiveTags(); track tag) {
                        <span
                          style="display:flex;align-items:center;gap:3px;padding:2px 7px;
                        ...
                        >
                          ...
                          <span
                            (click)="toggleTag(tag)"
                            style="cursor:pointer;font-size:13px;line-height:1;color:#B22F4E;"
                            >×</span
                          >
                        </span>
                      }
            ...
            <div style="position:relative;">
              ...
              >
                ...
                @if (manualEstimate() !== null) {
                  <span
                    (click)="clearManual($event, 'estimate')"
                    style="color:var(--mute);font-size:15px;line-height:1;cursor:pointer;"
                    >×</span
                  >
                }
              ...
              @if (activePicker() === 'estimate') {
                <div
                  style="position:absolute;left:0;top:calc(100% + 4px);z-index:56;background:var(--bg);border:1px solid var(--line);border-radius:10px;box-shadow:0 6px 24px rgba(0,0,0,.14);padding:10px;min-width:220px;"
                  (click)="$event.stopPropagation()"
                >
                  <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:4px;">
                    @for (p of estimatePresets; track p.minutes) {
                      <button
                        (click)="selectEstimate(p.minutes)"
                        style="padding:6px 4px;border-radius:7px;border:1.5px solid;
                                     font-size:12px;cursor:pointer;transition:all .12s;"
                        [style.border-color]="
                          manualEstimate() === p.minutes ? 'var(--orange)' : 'var(--line)'
                        "
                        [style.background]="
                          manualEstimate() === p.minutes ? 'var(--orange)' : 'transparent'
                        "
            ...
            <div style="position:relative;">
              ...
              >
                ...
                @if (manualRecurrence() !== null) {
                  <span
                    (click)="clearManual($event, 'recurrence')"
                    style="color:var(--mute);font-size:15px;line-height:1;cursor:pointer;"
                    >×</span
                  >
                }
              ...
              @if (activePicker() === 'recurrence') {
                <div
                  style="position:absolute;left:0;top:calc(100% + 4px);z-index:56;background:var(--bg);border:1px solid var(--line);border-radius:10px;box-shadow:0 6px 24px rgba(0,0,0,.14);padding:6px;min-width:180px;"
                  (click)="$event.stopPropagation()"
                >
                  @for (opt of recurrenceOptions; track opt.value) {
                    <button
                      ...
                      [style.background]="
                        effectiveRecurrence() === opt.value ||
                        (opt.value === '' && !effectiveRecurrence())
...
export class QuickAddComponent implements OnInit {
  ...
  filteredLabels = computed(() => {
    ...
    return q ? this.labels().filter((l) => l.name.toLowerCase().includes(q)) : this.labels();
  });
  ...
  effectiveProjectId = computed(
    () => this.manualProjectId() ?? this.ui.defaultProjectId() ?? this.inboxProject()?.id ?? null,
  );
  ...
  effectiveEstimate = computed<number | null>(() =>
    this.manualEstimate() !== null
      ? this.manualEstimate()
      : (this.parsed().estimateMinutes ?? null),
  );
  ...
  detected = computed<DetectedChip[]>(() => {
    ...
    if (p.tags.length)
      list.push({ k: 'tags', v: p.tags.map((t) => '#' + t).join(' '), color: '#3AA3FF' });
    ...
    if (p.estimateMinutes)
      list.push({ k: 'est', v: fmtEstimate(p.estimateMinutes), color: '#7AD36B' });
  ...
  togglePicker(name: PickerName): void {
    if (this.activePicker() === name) {
      this.activePicker.set(null);
      return;
    }

    if (name === 'date') {
      /* calendar navigation handled by DatetimePickerComponent */
    }
    if (name === 'tags') {
      this.tagSearch.set('');
  ...
  clearManual(e: MouseEvent, field: PickerName): void {
    ...
    switch (field) {
      case 'date':
        this.manualDatetime.set(null);
        break;
      case 'due':
        this.manualDueAt.set(null);
        break;
      case 'project':
        this.manualProjectId.set(null);
        break;
      case 'tags':
        this.manualTags.set(null);
        break;
      case 'estimate':
        this.manualEstimate.set(null);
        break;
      case 'recurrence':
        this.manualRecurrence.set(null);
        break;
    }
  }
